...
it was necessary to find a proper transformation technic between the deprecated
...
A common point set is defined by two sets of coordinates in GeoJson format,
one for the source coordinate system and one for the target coodinate system:
...
.. option:: +source=<geojsonfile> +target=<geojsonfile>
    ...
    ``++source`` and ``++target`` are links to geojson file with list of points.
    A point pair is a object with coordinates referred in both geodetic datums.
